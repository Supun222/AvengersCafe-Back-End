...
public interface FoodOrderRepository extends JpaRepository<FoodOrder,Integer> {
    ...
    @Query(value = "SELECT customerorder.cus_id, customerorder.table_num, food.name, food.preptime, foodorder.quantity FROM customerorder, food, foodorder " +
            "WHERE customerorder.act_id=5 AND customerorder.order_id=foodorder.order_id AND foodorder.food_id=food.food_id", nativeQuery = true)
    List<Object[]> retrivefoododers();
    ...
    @Query(value = "SELECT customerorder.order_id, foodorder.quantity, food.name, food.price FROM customerorder, food, foodorder " +
            "WHERE customerorder.cus_id=?1 AND customerorder.order_id=foodorder.order_id AND food.food_id=foodorder.food_id", nativeQuery = true)
